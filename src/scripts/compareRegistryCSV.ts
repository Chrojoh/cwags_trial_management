...
async function compareAndInsert(
  ...
  newRecords.slice(0, 10).forEach((record, idx) => {
    console.log(
      `   ${idx + 1}. ${record.cwags_number} - ${record.dog_call_name} (${record.handler_name})`
    );
  });
...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log(
      '  2. Compare only:    npx tsx src/scripts/compareRegistryCSV.ts <excel-csv> <db-csv>'
    );
    console.log(
      '  3. Compare & insert: npx tsx src/scripts/compareRegistryCSV.ts <excel-csv> <db-csv> --insert'
    );
    console.log('');
    console.log('Example:');
    console.log('  npx tsx src/scripts/compareRegistryCSV.ts --export');
    console.log(
      '  npx tsx src/scripts/compareRegistryCSV.ts public/excel_registry.csv public/db_registry.csv'
    );
    console.log(
      '  npx tsx src/scripts/compareRegistryCSV.ts public/excel_registry.csv public/db_registry.csv --insert'
    );
    process.exit(1);
  }
