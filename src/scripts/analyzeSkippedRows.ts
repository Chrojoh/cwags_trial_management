  ...
  try {
    ...
    for (let i = 0; i < rawData.length; i++) {
      ...
      if (!cwagsNumber || !dogCallName || !handlerName) {
        skippedRows.push({
          ...
          details:
            `Empty values: ${!cwagsNumber ? 'registry' : ''} ${!dogCallName ? 'call-name' : ''} ${!handlerName ? 'handler' : ''}`.trim(),
        });
        continue;
      }
    ...
    for (const [reason, rows] of skipReasons.entries()) {
      ...
      examples.forEach((example) => {
        console.log(
          `  Row ${example.rowNumber}: [${example.rowData
            .slice(0, 6)
            .map((cell) =>
    ...
    if (skippedRowNumbers.length > 0) {
      ...
      if (consecutiveBlocks.length > 0) {
        ...
        consecutiveBlocks.forEach((block, index) => {
          console.log(
            `  Block ${index + 1}: Rows ${block[0]}-${block[block.length - 1]} (${block.length} rows)`
          );
        });
      }
    }
