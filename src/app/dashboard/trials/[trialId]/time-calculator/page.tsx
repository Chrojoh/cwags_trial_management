...
export default function TrialTimeCalculatorPage() {
  ...
  const initializeDefaults = async () => {
    try {
      ...
      setError(
        `Failed to initialize defaults: ${err instanceof Error ? err.message : 'Unknown error'}`
      );
      throw err;
    }
  };
  ...
  return (
    <MainLayout title="Trial Time Calculator" breadcrumbItems={breadcrumbItems}>
        ...
        {hasInitialized && (
          <Alert className="bg-green-50 border-green-200">
            ...
            <AlertDescription className="text-green-800">
              Default time configurations have been initialized. You can now customize the minutes
              per run for each class.
            </AlertDescription>
          </Alert>
        )}
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {daysData.map((dayData) => {
                const totalMinutes = [...dayData.scent_configs, ...dayData.rally_configs].reduce(
                  (sum, config) => sum + config.total_minutes,
                  0
                );
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <CardTitle className="flex items-center space-x-2">
                      ...
                      <span>
                        Day {dayData.day_number} - {formatDate(dayData.trial_date)}
                      </span>
                      <Badge
                        variant={expandedDays.includes(dayData.day_id) ? 'default' : 'outline'}
                      >
                    ...
                    <CardDescription className="mt-1">
                      Total: {totalMinutes.toFixed(0)} min • Allotment: {dayData.allotted_minutes}{' '}
                      min •
                      <span
                        className={
                          isOvertime ? 'text-red-600 font-semibold' : 'text-green-600 font-semibold'
                        }
                      >
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="border-t pt-4">
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <Card
                        className={
                          isOvertime ? 'border-red-300 bg-red-50' : 'border-green-300 bg-green-50'
                        }
                      >
                        <CardContent className="pt-4">
                          <div className="text-sm text-gray-600">Remaining</div>
                          <div
                            className={`text-2xl font-bold ${isOvertime ? 'text-red-700' : 'text-green-700'}`}
                          >
                            {isOvertime ? '-' : '+'}
                            {Math.abs(remaining).toFixed(2)} min
                            {isOvertime && ' ⚠️'}
                          </div>
                        </CardContent>
                      </Card>
