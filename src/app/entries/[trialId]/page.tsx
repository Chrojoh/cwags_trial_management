...
export default function PublicEntryForm() {
    ...
    try {
      ...
      const allDesiredSelections = await Promise.all(
        formData.selected_rounds.map(async (roundId) => {
          ...
          const nextPosition =
            existingInRound && existingInRound.length > 0
  ...
  // GROUP ROUNDS BY DAY
  // ============================================
  const roundsByDay = trialRounds.reduce(
    (acc, round) => {
  ...
  if (success) {
    ...
    const selectedClassDetails = formData.selected_rounds
      .map((roundId) => {
        const round = trialRounds.find((r) => r.id === roundId);
            ...
            <Card className="max-w-md w-full">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button onClick={handleConfirmedSubmit} className="flex-1">
                    Confirm Update
                  </Button>
                </div>
              </CardContent>
