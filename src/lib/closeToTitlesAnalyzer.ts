...
function analyzeTrackerHistory(history: any[], className: string) {
  const levelRecords = history.filter(r => 
    r.class?.trim() === className.trim() && (r.qs || 0) > 0
  );
  ...
  const hasTitle = isGames
    ? totalQs >= requirements.minQs && judges.size >= requirements.minJudges && gameTypes.size >= (requirements.minGames || 0)
  ...
  if (hasTitle) {
    ...
    for (const record of sorted) {
      ...
      const nowHasTitle = isGames
        ? cumulativeQs >= requirements.minQs && judgesSoFar.size >= requirements.minJudges && gamesSoFar.size >= (requirements.minGames || 0)
        : cumulativeQs >= requirements.minQs && judgesSoFar.size >= requirements.minJudges;
      
      const previouslyHadTitle = isGames
        ? prevQs >= requirements.minQs && prevJudges >= requirements.minJudges && prevGames >= (requirements.minGames || 0)
      ...
      if (nowHasTitle && !previouslyHadTitle) {
        ...
        if (isGames && prevQs >= requirements.minQs && prevJudges >= requirements.minJudges && prevGames < (requirements.minGames || 0)) {
          qsTowardTitle = 1;
    ...
    try {
      ...
      for (const className of allClasses) {
        ...
        const projectedAceQs = trackerStatus.hasTitle
          ? trackerStatus.aceQs + entriesInClass
          : 0;
...
export async function generateCloseToTitlesReport(
  ...
  try {
    ...
    // -------------------------------------------------------
    const dogClassMap = new Map<string, {
      cwagsNumber: string;
      dogName: string;
      handlerName: string;
      className: string;
    ...
    entries.forEach(entry => {
      ...
      selections.forEach(sel => {
        ...
        // Get class name
        const round = Array.isArray(sel.trial_rounds) 
          ? sel.trial_rounds[0] 
          : sel.trial_rounds;
    ...
    for (const [key, dogClass] of dogClassMap) {
      try {
        ...
        const gamesNeededForTitle = (isGames && !trackerStatus.hasTitle)
        ...
        const willEarnTitle = !trackerStatus.hasTitle && 
          projectedQs >= requirements.minQs &&
          (trackerStatus.judgeCount >= requirements.minJudges || dogClass.roundCount >= requirements.minJudges);
    ...
    console.log(`✅ Report generated: ${report.closeToTitles.length} close to titles, ${report.closeToAces.length} close to aces`);
