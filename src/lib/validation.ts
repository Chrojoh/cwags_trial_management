...
export const phoneSchema = z.string().max(50, 'Phone number too long').optional().or(z.literal(''));
...
export const userCreateSchema = z.object({
  ...
  club_name: z.string().max(255, 'Club name too long').optional().or(z.literal('')),
...
export const judgeCreateSchema = z.object({
  ...
  phone: z.string().max(200, 'Phone number too long').optional().or(z.literal('')),

  city: z.string().max(100, 'City name too long').optional().or(z.literal('')),

  province_state: z.string().max(100, 'Province/State name too long').optional().or(z.literal('')),

  country: z.string().max(100, 'Country name too long').optional().or(z.literal('')),

  level: z.string().max(500, 'Level description too long').optional().or(z.literal('')),
...
export const registryCreateSchema = z.object({
  ...
  dog_call_name: z.string().min(1, 'Dog call name is required').max(100, 'Dog call name too long'),

  handler_name: z.string().min(1, 'Handler name is required').max(200, 'Handler name too long'),
...
export const trialCreateSchema = z
  .object({
    trial_name: z
      .string()
    ...
    trial_type: z.string().min(1, 'Trial type is required'),
    ...
    location: z.string().min(1, 'Location is required').max(255, 'Location description too long'),

    host_club: z.string().min(1, 'Host club is required').max(200, 'Host club name too long'),
