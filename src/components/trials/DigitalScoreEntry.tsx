      ...
      for (const entry of entries) {
        ...
        if (scoreSheetType === 'scent') {
          scoreData = {
            ...
            time_seconds: entry.time_seconds ? parseFloat(entry.time_seconds) : null,
            pass_fail: entry.pass_fail || null,
          };
        } else if (scoreSheetType === 'rally_obedience') {
          const numScore = entry.numerical_score ? parseFloat(entry.numerical_score) : null;
          scoreData = {
            numerical_score: numScore,
            pass_fail: entry.pass_fail || null,
          };
        } else if (scoreSheetType === 'games') {
          // For games, if Pass is selected, store the subclass symbol
       ...
       // ✅ Extract base round ID for Games classes with compound IDs
        ...
        if (!result.success) {
          ...
          throw new Error(`Failed to save score for ${entry.dogName}: ${result.error}`);
        }
      }
