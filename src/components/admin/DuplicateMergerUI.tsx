...
const DuplicateMergerUI = () => {
  ...
  const executeMerge = async () => {
    if (
      !confirm('Are you sure you want to merge these duplicate entries? This cannot be undone.')
    ) {
      return;
    }
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Duplicate Entry Merger</h1>
      ...
      {/* Trial ID Input */}
      <div className="bg-white rounded-lg shadow-md p-6">
        <label className="block text-sm font-medium text-gray-700 mb-2">Trial ID</label>
        <input
          type="text"
          value={trialId}
                      ...
                      >
                        <div className="flex items-center space-x-4 flex-1">
                          ...
                          <span className="text-sm text-gray-700 font-medium">
                            {entry.status.includes('PRIMARY')
                              ? 'PRIMARY'
                              : entry.status.split(' ').slice(1).join(' ')}
                          </span>
...
const ActionBadge: React.FC<ActionBadgeProps> = ({ action }) => {
  ...
  return (
    <span
      className={`px-2 py-1 text-xs font-medium rounded ${colors[action] || 'bg-gray-100 text-gray-800'}`}
    >
      {action}
    </span>
  );
