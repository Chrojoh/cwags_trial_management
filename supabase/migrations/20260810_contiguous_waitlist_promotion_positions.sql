-- Keep active running-order positions contiguous before appending a promoted dog.
-- This replaces the earlier max(position) + 1 behavior, which preserved gaps and
-- could describe the third active dog as position 6.
...
begin
  ...
  -- Preserve the existing relative order while closing historical gaps.
  with ordered as (
    select id,
           row_number() over (
  ...
  v_next_position := v_active_count + 1;
