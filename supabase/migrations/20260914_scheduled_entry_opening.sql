-- Optional automatic public-entry opening for C-WAGS trials.
-- Additive and safe for existing trials: null entry_open_at preserves the
-- existing manual Not Yet Open / Open / Closed behavior.
begin;

alter table public.trials
  add column if not exists entry_open_at timestamptz,
  add column if not exists entry_timezone text not null default 'America/Edmonton';

alter table public.trials
  drop constraint if exists trials_entry_timezone_check;

alter table public.trials
  add constraint trials_entry_timezone_check check (
    entry_timezone in (
      'America/St_Johns',
      'America/Halifax',
      'America/Toronto',
...
as $$
  select jsonb_build_object(
    'trial', jsonb_build_object(
      ...
      'entry_timezone', t.entry_timezone,
